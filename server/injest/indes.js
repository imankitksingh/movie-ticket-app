...
// inngest funtion to add userdata into mongoDB database
...
// inngest funtion to delete userdata into mongoDB database
...
const sendBookingConfirmationEmail = inngest.createFunction(
    ...
    async ({ event, step }) => {
        try {
            ...
            if (!booking.user?.email) throw new Error("User email not found");
          ...
          <div style="font-family: Arial, sans-serif; line-height: 1.5;">
            ...
            console.log("✅ Email sent response:", response?.messageId || response);

        } catch (err) {
            console.error("Error sending confirmation email:", err);
            throw err; // Let Inngest know it failed so it can retry or log
        }
    }
);

//inngest funtion to send email for show reminder
...
// funtion to let every user new whenever any show will be added by admin
...
export const functions = [syncUserCreation, syncUserDeletion, syncUserUpdation, relaeaseSeatsAndDeleteBooking, sendBookingConfirmationEmail, sendShowReminder, sendNewShowNotifications];
